import { writeAttribute, startAttribute, startTag, writeTextChild, writeInterpolatedChildren, writeSpreadAttribute, closeElement, getStackTrace, } from "./context.js";
function invalidMessage(context, expected) {
    return (`Invalid token "${context.token.type}" ` +
        `expected ${expected}.` +
        getStackTrace(context));
}
export const bodyOrTagOpener = function bodyOrTagOpener(context) {
...
export const closeMarkerOrTagName = function closeMarkerOrTagName(context) {
...
export const tagNameOrTagCloser = function tagNameOrTagCloser(context) {
...
export const attributeName = function attributeName(context) {
...
export const quouteOrInterpolation = function quouteOrInterpolation(context) {
    ...
    throw new Error(invalidMessage(context, 'opening quotation mark (") or value'));
};
export const valueOrQuote = function valueOrQuote(context) {
    ...
    throw new Error(invalidMessage(context, 'value or closing quotation mark (")'));
};
export const quote = function quote(context) {
    if (context.token.type === 'quote') {
        return attributeName;
    }
